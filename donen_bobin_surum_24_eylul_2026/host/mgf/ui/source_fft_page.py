"""
Dedicated Signal Source FFT / Order Analysis Widget for MGF Radar V2.
Allows selecting different signal sources (ADC, Encoder Rate, Step Rate, Ratio)
for live analysis and calculation of FFT of recorded data.
"""
import numpy as np
...
class SourceFftAnalysisWidget(QWidget):
    def __init__(self, engine, parent=None):
        ...
        lbl_guide = QLabel(
            "1. Choose the Signal Source from the settings panel.\n"
            "2. Tune peak threshold to auto-label peaks.\n"
            "3. Capture spectrum comparison slots to overlay live profiles."
        )
        lbl_guide.setWordWrap(True)
        lay_guide.addWidget(lbl_guide)
        self.sect_guide = CollapsibleSection("Signal Source FFT Guide", grp_guide)
        ...
        lay_set.addWidget(QLabel("Signal Source:"), 0, 0)
        self.cb_signal_source = QComboBox()
        self.cb_signal_source.addItems([
            "ADC Signal", 
            "Encoder Rate", 
            "Step Rate", 
            "Encoder/Step Rate Ratio"
        ])
        self.cb_signal_source.currentTextChanged.connect(self.force_recompute_fft)
        lay_set.addWidget(self.cb_signal_source, 0, 1)

        lay_set.addWidget(QLabel("FFT Window:"), 1, 0)
        self.cb_fft_window = QComboBox()
        self.cb_fft_window.addItems(["Hanning", "Hamming", "Blackman", "Rectangular"])
        lay_set.addWidget(self.cb_fft_window, 1, 1)
        
        lay_set.addWidget(QLabel("Domain:"), 2, 0)
        self.cb_fft_domain = QComboBox()
        self.cb_fft_domain.addItems(["Order", "Frequency"])
        lay_set.addWidget(self.cb_fft_domain, 2, 1)
        
        lay_set.addWidget(QLabel("Max X Limit:"), 3, 0)
        self.txt_fft_max_x = QLineEdit("100")
        lay_set.addWidget(self.txt_fft_max_x, 3, 1)
        
        lay_set.addWidget(QLabel("Y Scale:"), 4, 0)
        ...
        lay_set.addWidget(self.cb_fft_y_mode, 4, 1)
        
        lay_set.addWidget(QLabel("Peak Thresh (dB):"), 5, 0)
        self.txt_threshold_db = QLineEdit("-40.0")
        lay_set.addWidget(self.txt_threshold_db, 5, 1)
        
        lay_set.addWidget(QLabel("Max Peaks:"), 6, 0)
        self.txt_max_peaks = QLineEdit("8")
        lay_set.addWidget(self.txt_max_peaks, 6, 1)
        
        lay_set.addWidget(QLabel("Zero-Pad:"), 7, 0)
        self.cb_zero_pad = QComboBox()
        self.cb_zero_pad.addItems(["1x (Off)", "2x", "4x"])
        lay_set.addWidget(self.cb_zero_pad, 7, 1)
        
        lay_set.addWidget(QLabel("Averaging:"), 8, 0)
        self.cb_averaging = QComboBox()
        self.cb_averaging.addItems(["1 (Off)", "2", "4", "8"])
        lay_set.addWidget(self.cb_averaging, 8, 1)
        
        lay_set.addWidget(QLabel("FFT Samples:"), 9, 0)
        self.txt_fft_samples = QLineEdit("16384")
        lay_set.addWidget(self.txt_fft_samples, 9, 1)
        
        lay_set.addWidget(QLabel("Jitter Tolerance:"), 10, 0)
        self.txt_jitter_tolerance = QLineEdit("0.08")
        lay_set.addWidget(self.txt_jitter_tolerance, 10, 1)
        
        self.chk_div_speed = QCheckBox("Divide X by Measured Speed")
        self.chk_div_speed.setChecked(True)
        lay_set.addWidget(self.chk_div_speed, 11, 0, 1, 2)
        
        self.chk_dc_remove = QCheckBox("Remove DC in Time Plot")
        self.chk_dc_remove.setChecked(False)
        lay_set.addWidget(self.chk_dc_remove, 12, 0, 1, 2)
        ...
        lay_set.addWidget(self.btn_clear_buffer, 13, 0, 1, 2)
        
        self.sect_settings = CollapsibleSection("FFT & Source Settings", grp_settings)
        ...
        lbl_title = QLabel("Signal Source FFT Spectrum & Order Analysis")
    ...
    def load_custom_labels(self):
        import json
        import os
        if os.path.exists("source_fft_custom_labels.json"):
            try:
                with open("source_fft_custom_labels.json", "r", encoding="utf-8") as f:
                    data = json.load(f)
    ...
    def save_custom_labels(self):
        import json
        try:
            with open("source_fft_custom_labels.json", "w", encoding="utf-8") as f:
    ...
    def update_data(self, adc, rate):
        ...
        # Extract time_data according to the chosen signal source
        source = self.cb_signal_source.currentText()
        if source == "ADC Signal":
            _, time_data = self.engine.get_latest_data(num_samples)
            if len(time_data) < 128:
                return
        else:
            # We need num_samples + 1 raw samples to get num_samples difference rates
            enc_pulses, step_pulses = self.engine.get_latest_pulses(num_samples + 1)
            if len(enc_pulses) < 128:
                return
                
            if source == "Encoder Rate":
                time_data = np.diff(enc_pulses) * rate
            elif source == "Step Rate":
                time_data = np.diff(step_pulses) * rate
            elif source == "Encoder/Step Rate Ratio":
                enc_rate = np.diff(enc_pulses) * rate
                step_rate = np.diff(step_pulses) * rate
                time_data = np.zeros_like(enc_rate)
                valid = step_rate != 0
                time_data[valid] = enc_rate[valid] / step_rate[valid]
            else:
                return

        # Handle DC Removal setting
        if self.chk_dc_remove.isChecked():
        ...
        if hasattr(win, 'executor') and win.executor is not None:
            if self._future is None:
                ...
                self._future = win.executor.submit(
                    run_background_fft,
                    time_data, rate, motor_speed, w_type, -1, "Frequency",
            ...
            freqs, mag = self.analyzer.perform_fft(
                time_data, rate, motor_speed, window_type=w_type, domain="Frequency",
    ...
    def take_fft_screenshot(self):
        ...
        filepath = os.path.join("outputs", "screenshots", f"source_fft_plot_{ts}.png")
